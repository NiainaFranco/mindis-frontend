...
export class RibonColorSetService {
  ...
  getAllWithPagination(query: { page: number; name?: string }) {
    return this.httpClient.get<PaginationMetaWrapper<GetRibonColorSetType>>(
      '/ribon-color-set/get-all',
      {
        params: query,
      },
    );
  ...
  createRibonColorSet(arg: { ribonColorSet: CreateRibonColorSetType }) {
    return this.httpClient.post<GetRibonColorSetType>('/ribon-color-set/create', arg.ribonColorSet);
  }

  deleteRibonColorSet(arg: { ribonColorSetId: string }) {
    return this.httpClient.delete(`/ribon-color-set/delete/${arg.ribonColorSetId}`);
  }

  updateRibonColorSet(arg: { ribonColorSet: UpdateRibonColorSetType }) {
